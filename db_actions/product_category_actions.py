...
class ProductCategoryActions:
    ...
    def get_categories(params: IProductCategoryQueryParams):
        ...
        rows = (
            ...
            .where(
                ProductCategory.category_name.contains(params.name)
                if params.name
                else True
            )
            .order_by(*sort_by)
    ...
    def get_category_by_id(id_: int):
        rows = (
            ProductCategory.select().where(ProductCategory.category_id == id_).dicts()
        )
    ...
    def get_category_by_name(name: str):
        rows = (
            ProductCategory.select()
            .where(ProductCategory.category_name == name)
            .dicts()
        )
        return [row for row in rows]
    ...
    @staticmethod
    def update_category(
        category_id: int, category: IProductCategoryUpdate, current_user: int
    ):
        category_ = {x: y for x, y in category.dict().items() if y != None}.copy()
        ProductCategory.update(**category_).where(
    ...
    def get_category_items(
        category_id: int, current_user: int, params: IProductQueryParams
    ):
        ...
        __rows = (
            ...
            .where(
                (ProductCategoryRef.category_id == category_id)
                & (
    ...
    def add_prod_category(cat_prod_ref: IProductCategoryRef, current_user: int):
        ...
        return {
            "add": (
                f"Prod: {cat_prod_ref.product_id} === Cat: {cat_prod_ref.category_id}"
            )
        }
    ...
    def remove_prod_from_category(cat_prod_ref: IProductCategoryRef, current_user: int):
        ...
        return {
            "removed": f"Prod: {cat_prod_ref.product_id} === Cat: {cat_prod_ref.category_id}"
        }
