...
from schemas.iproduct_category import IProductCategory, IProductCategoryQueryParams, IProductCategoryRefAdd, IProductCategoryUpdate, IProductCategoryRef
...
@router.post("/")
async def create_product_category(category: IProductCategory,
...
async def add_product_to_category(category_id: int, product: IProductCategoryRefAdd, current_user: ITokenData = Depends(get_current_user)):
    cat_prod_ref: IProductCategoryRef = IProductCategoryRef(
        category_id=category_id, product_id=product.product_id)
    try:
        return ProductCategoryActions.add_prod_category(cat_prod_ref, current_user.user_id)
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail={"message": f"{e}"}
...
async def get_category_products(
    ...
    try:
        return ProductCategoryActions.get_category_items(category_id, current_user.user_id)
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail={
